...
def volatility(ticker, log_retornos_periodo=5, limite=0.25, capital=500):
    ...
    mudanca_sinal = (log_retornos_acumulados > 0) & (log_retornos_acumulados.shift(1) < 0) | (
                log_retornos_acumulados < 0) & (log_retornos_acumulados.shift(1) > 0)
    sinais[compra.values] = 1
    sinais[venda.values] = -1
    sinais[mudanca_sinal.values] = 0
    ...
    return retornos_portifolio, resultado_acumulado, sharpe_ratio
